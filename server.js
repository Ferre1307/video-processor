...
const app = express();
app.use(express.json({ limit: "100mb" }));const express = require("express");
const { exec } = require("child_process");
const fs = require("fs");
const path = require("path");
const https = require("https");
...
async function generateVoice(text, audioPath, voice = "es-PY-TaniaNeural", srtPath = null) {
  ...
  if (segFiles.length === 1) {
    ...
    // Mover también el SRT del segmento único al srtPath esperado por FFmpeg
    const finalSrtPath = srtPath || audioPath.replace(".mp3", ".srt");
    const segSrt = segFiles[0].replace(".mp3", ".srt");
    if (fs.existsSync(segSrt)) fs.renameSync(segSrt, finalSrtPath);
    return audioPath;
  }
  ...
  if (srtContent.trim()) fs.writeFileSync(finalSrtPath, srtContent, "utf8");
...
app.post("/process-video", async (req, res) => {
  ...
  try {
    ...
    await generateVoice(text, audioPath, voice, srtPath);
